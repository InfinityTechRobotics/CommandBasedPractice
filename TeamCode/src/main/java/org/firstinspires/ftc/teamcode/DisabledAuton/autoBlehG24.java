...
// Note: NONE OF THE POSES HERE HAVE BEEN CHANGED YET!!!!!!!!!!

@Disabled
@Autonomous
    public class autoBlehG24 extends OpMode {
...
//    private final Pose viewPose = new Pose(80, 120, Math.toRadians(90)); // Pose to read the Obelisk.
    ...
    private final Pose pickupPose21 = new Pose(128, 41, Math.toRadians(0)); // Last (Third Set) of Artifacts from the Spike Mark(GPP).

    private final Pose prePickupPose22 = new Pose(92, 67, Math.toRadians(0)); // Preparing to intake second set of artifacts.

    private final Pose pickupPose22 = new Pose(120, 65, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark(PGP).
    ...
    private  final Pose endPose = new Pose(90, 67, Math.toRadians(0));//its the end - if you took the time to read this, you get it - otherwise vid is dissapointed :(
                ...
                if (result.isValid()) {
                    ...
                    for (LLResultTypes.FiducialResult fr : fiducialResults) {
                        ...
                        if (obeliskResult == DESIRED_TAG_ID_1 || obeliskResult == DESIRED_TAG_ID_2 || obeliskResult == DESIRED_TAG_ID_3 && opmodeTimer.getElapsedTimeSeconds() > START_DELAY_TIME) {
                            setPathState(11); // No matter what motif it is, we go score our preloaded artifacts
                            ...
                            setPathState(11); // We go score our preloaded artifacts
                        } else {
                    ...
                    setPathState(11); // Goes to score preloaded artifacts
                }
                else {
