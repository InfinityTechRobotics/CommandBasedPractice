...
@Disabled
@Autonomous
    public class theThreenthPedroOfDecodeWithLimeLightREDSRVGATINHF extends OpMode {
...
//    private final Pose viewPose = new Pose(80, 120, Math.toRadians(90)); // Pose to read the Obelisk.
    ...
    private final Pose pickup1Pose = new Pose(124, 92, Math.toRadians(0)); // Highest (First Set) of Artifacts from the Spike Mark(PPG).

    private final Pose prePickupPose2 = new Pose(100, 70, Math.toRadians(0)); // Preparing to intake second set of artifacts.
    ...
    double flapREngaged = 0.22;
    double flapRDisengaged = 0.08;
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 11: // shooting the preloaded
                if(pathTimer.getElapsedTimeSeconds() > 0.25 && pathTimer.getElapsedTimeSeconds() < 1.0) {
                    servoPaddleLeft.setPosition(flapLEngaged);
                    servoPaddleRight.setPosition(flapREngaged);
                }
                if(pathTimer.getElapsedTimeSeconds() > 2 && pathTimer.getElapsedTimeSeconds() < 3) {
//                      servoPaddleLeft.setPosition(flapLDisengaged);
                    servoPaddleRight.setPosition(flapRDisengaged);
                }
...
//                      servoPaddleLeft.setPosition(flapLDisengaged);
                ...
                if(pathTimer.getElapsedTimeSeconds() > 9 && pathTimer.getElapsedTimeSeconds() < 10) {
                    motorTransfer.setPower(transferOff);
//                    servoPaddleLeft.setPosition(flapLEngaged);
                    servoPaddleRight.setPosition(flapREngaged);
                }
