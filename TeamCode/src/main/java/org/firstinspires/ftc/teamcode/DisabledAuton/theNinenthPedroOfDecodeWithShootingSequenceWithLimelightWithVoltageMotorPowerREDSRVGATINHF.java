...
@Disabled
@Autonomous
    public class theNinenthPedroOfDecodeWithShootingSequenceWithLimelightWithVoltageMotorPowerREDSRVGATINHF extends OpMode {
...
//    double flyWheelSpeedOff = 0;
    ...
    double SERVO_STOP_CLOSE_POS = 0.4;
...
//                        servoPaddleRight.setPosition(flapLEngaged);
                ...
                break;
            case 10001:
                if(pathTimer.getElapsedTimeSeconds() > 0.25) { // lowers the flaps while finishing the action
...
//                        servoPaddleRight.setPosition(flapLDisengaged);
            ...
            case 10003:
                if(pathTimer.getElapsedTimeSeconds() > 1) { // stops the transfer after the prior action completed & shoots the next artifact
                    ...
                    servoStop.setPosition(SERVO_STOP_OPEN_POS);
                    // shuts down transfer to prevent a jam
                    servoPaddleLeft.setPosition(flapLEngaged); // extends the flaps to complete the action
//                        servoPaddleRight.setPosition(flapLEngaged);
                    setPathState(10004);
                }
...
//                        servoPaddleRight.setPosition(flapLDisengaged);
            ...
            case 10005:
                ...
                break;
            case 10006:
                if(pathTimer.getElapsedTimeSeconds() > 3) { // stops the transfer after the prior action completed & shoots the next artifact
                    motorTransfer.setPower(transferOff);
                    servoStop.setPosition(SERVO_STOP_OPEN_POS);
                    servoPaddleLeft.setPosition(flapLEngaged); // extends the flaps to complete the action
//                        servoPaddleRight.setPosition(flapLEngaged);
            ...
            case 10008:
                setPathState(10009);
                break;
            case 10009: // last state, just stops and waits while shutting down
                if (pathTimer.getElapsedTimeSeconds() > 0.1 && pathTimer.getElapsedTimeSeconds() < 0.15 ) {
                    if (obeliskResult == 23) {
                ...
                if (pathTimer.getElapsedTimeSeconds() > 0.2 && pathTimer.getElapsedTimeSeconds() < 0.25 ) {
