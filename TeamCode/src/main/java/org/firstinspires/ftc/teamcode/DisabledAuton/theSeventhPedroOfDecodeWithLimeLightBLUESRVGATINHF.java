...
@Disabled
@Autonomous(name = "SeventhPedroTestWithLimelightSRVGATINHFBlue", group = "pedroPathing")
    public class theSeventhPedroOfDecodeWithLimeLightBLUESRVGATINHF extends OpMode {
...
//    private final Pose viewPose = new Pose(80, 120, Math.toRadians(90)); // Pose to read the Obelisk.
    ...
    private final Pose scorePose = new Pose(64, 102, Math.toRadians(135)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.

    private final Pose prePickupPose1 = new Pose(44, 90, Math.toRadians(180)); // Preparing to intake first set of artifacts.

    private final Pose pickup1Pose = new Pose(20, 90, Math.toRadians(180)); // Highest (First Set) of Artifacts from the Spike Mark(PPG).

    private final Pose prePickupPose2 = new Pose(44, 68, Math.toRadians(180)); // Preparing to intake second set of artifacts.

    private final Pose pickup2Pose = new Pose(20, 68, Math.toRadians(180)); // Middle (Second Set) of Artifacts from the Spike Mark(PGP).

    private final Pose prePickupPose3 = new Pose(44, 44, Math.toRadians(180)); // Preparing to intake third set of artifacts.

    private final Pose pickup3Pose = new Pose(20, 44, Math.toRadians(180)); // Last (Third Set) of Artifacts from the Spike Mark(GPP).

    private final Pose controlPoint1 = new Pose(109, 55); // Control point - you get the idea - read the name
...
//                .build();
        // moves from the start to score position to shoot preloaded
        ...
        // moves from score position to pre-pickup-1
        driveToPrePickup1 = follower.pathBuilder()
                ...
                .setTimeoutConstraint(0)
                .build();
        // collects the artifacts at pickup one by moving over spikemark-1 (from pre-pickup-1 to pickup-1)
        driveToPickup1 = follower.pathBuilder()
                ...
                .setTimeoutConstraint(0)
                .build();
        // moves from pickup-1 to score pose to shoot pickup-1 artifacts
        ...
        // moves from the start position to pre-pickup-2
        driveToPrePickup2 = follower.pathBuilder()
                ...
                .setTimeoutConstraint(0)
                .build();
        // collects the artifacts at pickup-1 by moving over spikemark-1 (from pre-pickup-2 to
