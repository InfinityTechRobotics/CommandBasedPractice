...
@Disabled
@Autonomous
    public class autoRadG23 extends OpMode {
...
//    private final Pose viewPose = new Pose(80, 120, Math.toRadians(90)); // Pose to read the Obelisk.
    ...
    private final Pose motifDetection = new Pose(85, 120, Math.toRadians(90));
    ...
    private final Pose prePickupPose21 = new Pose(100, 44, Math.toRadians(0)); // Preparing to intake third set of artifacts.

    private final Pose pickupPose21 = new Pose(128, 44, Math.toRadians(0)); // Last (Third Set) of Artifacts from the Spike Mark(GPP).

    private final Pose prePickupPose22 = new Pose(100, 68, Math.toRadians(0)); // Preparing to intake second set of artifacts.

    private final Pose pickupPose22 = new Pose(124, 68, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark(PGP).

    private final Pose prePickupPose23 = new Pose(100, 91.75, Math.toRadians(0)); // Preparing to intake first set of artifacts.

    private final Pose pickupPose23 = new Pose(124, 91.75, Math.toRadians(0)); // Highest (First Set) of Artifacts from the Spike Mark(PPG).
    ...
    private PathChain driveToMotif, driveToGoal, driveToPrePickup23, driveToPickup23, driveToGoal23, driveToPrePickup22, driveToPickup22, driveToGoal22, driveToPrePickup21, driveToPickup21, driveToGoal21, driveToEnd;
...
//    double flyWheelSpeedOff = 0;
    ...
    double SERVO_PADDLE_SHOOT_POS = 0.6;
...
//                .build();
        driveToMotif = follower.pathBuilder()
                .addPath(new BezierLine(startPose, motifDetection))
                .setLinearHeadingInterpolation(startPose.getHeading(), motifDetection.getHeading())
                .setTimeoutConstraint(0)
                .build();

        driveToGoal = follower.pathBuilder()
                .addPath(new BezierLine(motifDetection, scorePose))
                .setLinearHeadingInterpolation(motifDetection.getHeading(), scorePose.getHeading())
                .setTimeoutConstraint(0)
                .build();
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 10: // drives toward goal to score preloaded artifacts
                ...
                follower.followPath(driveToMotif);
            ...
            case 10010:  // sends to next driving path
                if (pathTimer.getElapsedTimeSeconds() > 0.1 ) {
                    if ((shootingSequenceFlag == 23) && (endShootingSequenceFlag == 0)) {
                        ...
                        setPathState(210);
                        endShootingSequenceFlag += 1;
                    } else if (endShootingSequenceFlag == 2) {
                        setPathState(777);
                    } else if ((path2 == 1) && (endShootingSequenceFlag == 1)) {
                        setPathState(210);
                    } else if ((path2 == 2) && (endShootingSequenceFlag == 1) ) {
                        setPathState(220);
                    }
                    motorTransfer.setPower(transferOn);
                }
