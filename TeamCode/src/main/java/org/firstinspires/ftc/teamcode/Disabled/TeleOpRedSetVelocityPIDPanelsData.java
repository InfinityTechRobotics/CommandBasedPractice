...
public class TeleOpRedSetVelocityPIDPanelsData extends OpMode {
    ...
    private AnalogInput batteryAmp;
    ...
    PIDFCoefficients pidfOrig, pidfModified;
    ...
    public void init() {
        ...
        batteryAmp = hardwareMap.get(AnalogInput.class, "batteryAmp");
        ...
        pidfOrig = motorFlywheel.getPIDFCoefficients(DcMotorEx.RunMode.RUN_USING_ENCODER);

        // Open the log file for appending (true flag)
        try {
            String logFilePath = String.format("%s/FIRST/DataLogs/DataRPMLogOBJ_Nov14_01.csv", Environment.getExternalStorageDirectory().getPath());
            logWriter = new FileWriter(logFilePath, true);
            logWriter.write("Timer (s), Target RPM, Actual RPM, Flywheel Motor Power, Battery Voltage (V), Amperage (A)\n");  // CSV header
        } catch (IOException e) {
            telemetry.addData("Error", "Failed to open log file: " + e.getMessage());
        }

        // logWriter.write(currentTime + "," + targetRPM + "," + flywheelRPM + "," + motorFlywheel.getPower() + "," + battery.getVoltage() + "," + amperage + "\n");
    ...
    public void loop() {
        ...
        if (targetFound) {
            ...
            telemetry.addData("\n>", "Target Not Found\n");
            distanceToGoalInches = 64.;
        }

        if (Math.abs(error) > 1.0) {
            newPos = currentPos + error * 0.0015;   // 0.0004
        } else {
            newPos = currentPos;
        }
        ...
        if (i % 20 == 0) {
            ...
            amperage = batteryAmp.getMaxVoltage()*(50./3.3);
        ...
        telemetry.addData("Amperage", amperage);
        ...
        telemetry.addData("P,I,D,F (orig)", "P: %.4f, I: %.4f, D: %.4f, F: %.4f",
                pidfOrig.p, pidfOrig.i, pidfOrig.d, pidfOrig.f);
        telemetry.addData("P,I,D,F (modified)", "P: %.4f, I: %.4f, D: %.4f, F: %.4f",
                pidfModified.p, pidfModified.i, pidfModified.d, pidfModified.f);
//        telemetry.addData("P (orig)", pidfOrig.p);
//        telemetry.addData("I (orig)", pidfOrig.i);
//        telemetry.addData("D (orig)", pidfOrig.d);
//        telemetry.addData("F (orig)", pidfOrig.f);
//        telemetry.addData("P (modified)", pidfModified.p);
//        telemetry.addData("I (modified)", pidfModified.i);
//        telemetry.addData("D (modified)", pidfModified.d);
//        telemetry.addData("F (modified)", pidfModified.f);
        ...
        panelsTelemetry.addData("Flywheel Motor Power", motorFlywheel.getPower());
        panelsTelemetry.addData("Battery Voltage", battery.getVoltage());
        panelsTelemetry.addData("Amperage", amperage);
