...
//@Configurable
@TeleOp
public class TeleOpRedStatesLimiterTest extends OpMode {
...
//        panelsTelemetry.update(telemetry);
    ...
    public void loop() {
        ...
        // Driver Controls
        double currentPower = gamepad1.left_stick_y;
...
//        panelsTelemetry.update(telemetry);
    ...
    double limiter(double currentPower, double previousPower) {
        if (Math.abs(currentPower - previousPower) > MAX_CHANGE) { // Can slew
        ...
        return currentPower; // Close enough that you can just use input
    }



        public void tankMoveRobot (double y) {
            this.y = y;

            double frontLeftPower = y * limiter(currentPower, previousPower);
            double backLeftPower = y * limiter(currentPower, previousPower);
            double frontRightPower = y * limiter(currentPower, previousPower);
            double backRightPower = y * limiter(currentPower, previousPower);
