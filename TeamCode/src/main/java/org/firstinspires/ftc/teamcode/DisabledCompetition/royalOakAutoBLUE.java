...
@Disabled
@Autonomous
    public class royalOakAutoBLUE extends OpMode {
...
// Start Pose of our robot.
    ...
    private final Pose scorePose = new Pose(54, 98, Math.toRadians(135));
// Scoring Pose of our robot. It is facing the goal at a 135 degree angle.

    private  final Pose endPose = new Pose(54, 70, Math.toRadians(180));
// Ending Pose of our robot after finishing.

    private final Pose prePickupPose1 = new Pose(50, 84, Math.toRadians(180));
// Preparing to intake the first set of artifacts.

    private final Pose pickup1Pose = new Pose(26, 84, Math.toRadians(180));
// Highest (First Set) of Artifacts from the Spike Mark(PPG).

    private final Pose prePickupPose2 = new Pose(50, 58, Math.toRadians(180));
// Preparing to intake the second set of artifacts.

    private final Pose pickup2Pose = new Pose(22, 58, Math.toRadians(180));
// Middle (Second Set) of Artifacts from the Spike Mark(PGP).

    private final Pose prePickupPose3 = new Pose(50, 34, Math.toRadians(180));
// Preparing to intake the third set of artifacts.

    private final Pose pickup3Pose = new Pose(22, 34, Math.toRadians(180));
// Last (Third Set) of Artifacts from the Spike Mark(GPP).

    private final Pose controlPoint1 = new Pose(90, 0);
